        ...
        <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm animate-in zoom-in-95">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-6 mb-6 gap-4">
            ...
            <div className="flex gap-3 w-full md:w-auto">
              <button onClick={() => printBill(bill, bill.customers, items)} className="flex-1 md:flex-none bg-slate-900 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-lg">🖨️ প্রিন্ট বিল</button>
              <button onClick={() => printBill(bill, bill.customers, items)} className="flex-1 md:flex-none bg-green-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg">📥 PDF ডাউনলোড</button>
            </div>
          </div>
          {/* ... বাকি কাস্টমার এবং টেবিল কোড ChalanManager এর মতই থাকবে */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Customer Info</p><p className="font-bold text-slate-800 text-lg">{bill.customers?.name}</p><p className="font-bold text-slate-500">{bill.customers?.phone}</p><p className="text-sm text-slate-400 italic">📍 {bill.customers?.address}</p></div>
          ...
          <table className="w-full text-left mb-6">
            <thead className="text-[10px] uppercase font-black text-slate-400 border-b"><tr><th className="pb-4">Product Details</th><th className="pb-4 text-center">Qty</th><th className="pb-4 text-right">Price</th><th className="pb-4 text-right pr-0">Total</th></tr></thead>
            <tbody>{items.map((item, idx) => (<tr key={idx} className="border-b last:border-0"><td className="py-4 font-bold">{item.products?.name} <br/><span className="text-xs text-slate-400">{item.products?.model}</span></td><td className="text-center font-black">{item.quantity}</td><td className="text-right font-medium">{item.unit_price} ৳</td><td className="text-right font-black pr-0">{item.total_price} ৳</td></tr>))}</tbody>
            <tfoot><tr><td colSpan="3" className="pt-6 text-right font-black text-slate-400 uppercase">Grand Total</td><td className="pt-6 text-right text-3xl font-black text-green-600 pr-0">{bill.total_amount} ৳</td></tr></tfoot>
          </table>
        </div>
      )}
    </div>
