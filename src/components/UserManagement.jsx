...
const getAuthAdminClient = () => {
  if (!authAdminInstance) {
    ...
    authAdminInstance = createClient(supabaseUrl, supabaseServiceKey, {
      auth: {
...
const UserManagement = () => {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      // Auth একাউন্ট তৈরি
      const { error: authError } = await getAuthAdminClient().auth.signUp({
        email: newUser.email.trim(),
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      const { data: authUsers } = await getAuthAdminClient().auth.admin.listUsers();
      if (authUsers && authUsers.users) {
        const authUser = authUsers.users.find(u => u.email === editForm.originalEmail);
        if (authUser) {
          await getAuthAdminClient().auth.admin.updateUserById(authUser.id, {
  ...
  const deleteUser = async (userId, userEmail, empName) => {
    ...
    setLoading(true);
    try {
      const { data: authUsers, error: listErr } = await getAuthAdminClient().auth.admin.listUsers();
      if (listErr) throw new Error("Service Role Key কাজ করছে না!");
      
      if (authUsers && authUsers.users) {
        const authUser = authUsers.users.find(u => u.email === userEmail);
        if (authUser) await getAuthAdminClient().auth.admin.deleteUser(authUser.id);
      }
